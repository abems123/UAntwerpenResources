...
void CFG::sortProductions()
{
    ...
    /*
    std::sort(productions.begin(), productions.end(), [](const Production* a, const Production* b)
    {
        return (a->getHead() + " -> " + Utils::body_to_string(a->getBody())) < (b->getHead() + " -> " +
            Utils::body_to_string(b->getBody()));
    });
...
CFG::CFG(const string& json_file)
{
    ...
    {
        auto new_production = new Production();
        new_production->setHead(p["head"]);
        new_production->setBody(p["body"].get<vector<string>>());
...
pair<string, bool> CFG::hasUniqueProduction(const string& a)
{
...
void CFG::replaceTerminals()
{
    ...
    cout << "  Created " << new_productions << " new productions, original had " << original_productions << endl << endl;
...
string CFG::createTwoProduction(const int number, const string& head, const vector<string>& body, vector<string>& new_variables)
{
...
void CFG::breakBodies()
{
    ...
    variables.insert(variables.end(), new_variables.begin(), new_variables.end());
    cout << endl << " >> Broke " << broken_bodies << " bodies, added " << new_variables.size() << " new variables" << endl;
