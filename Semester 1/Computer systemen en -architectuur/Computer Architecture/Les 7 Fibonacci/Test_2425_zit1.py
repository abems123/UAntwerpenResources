...
# (name, parse_pattern, help_string, compile_function)
FULL_DP_OPERATIONS = [
('brnz'  , pattern((('op', 'brnz'),  ('rd', register()), ('imm', integerpattern())), can_have_label=True),                          "brnz rd imm     -->  rd!=0 ? pc := pc+1+imm",      lambda m, params : '101'+str(reg2bin(m.group('rd'),REGISTER_ADDRESS_SIZE))+str(int2bin(m.group('imm'),9))+'0' ),
('brnz'  , pattern((('op', 'brnz'),  ('rd', register()), ('to', labelpattern())), can_have_label=True),                             "brnz rd label   -->  rd!=0 ? pc := pc+1+imm",      lambda m, params : '101'+str(reg2bin(m.group('rd'),REGISTER_ADDRESS_SIZE))+str(label2bin(m.group('to'),9,params[1],params[0]-1))+'0' ),
('jr'   , pattern((('op', 'jr'),   ('rd', register()), ('uimm', binarypattern(9))), can_have_label=True),                           "jr rd uimm      -->  pc := rd+uimm",               lambda m, params : '101'+str(reg2bin(m.group('rd'),REGISTER_ADDRESS_SIZE))+m.group('uimm')+'1' ),
('jr'   , pattern((('op', 'jr'),   ('rd', register()), ('uimm', uintegerpattern())), can_have_label=True),                          "jr rd uimm      -->  pc := rd+uimm",               lambda m, params : '101'+str(reg2bin(m.group('rd'),REGISTER_ADDRESS_SIZE))+str(uint2bin(m.group('uimm'),9))+'1' ),
('jr'   , pattern((('op', 'jr'),   ('rd', register()), ('to', labelpattern())), can_have_label=True),                               "jr rd label     -->  pc := rd+imm",                lambda m, params : '101'+str(reg2bin(m.group('rd'),REGISTER_ADDRESS_SIZE))+str(label2bin(m.group('to'),9,params[1]))+'1' ),

('j'    , pattern((('op', 'j'),    ('addr', binarypattern(12))), can_have_label=True),                                              "j addr          -->  pc := addr",                  lambda m, params : '110'+m.group('addr')+'0' ),
('j'    , pattern((('op', 'j'),    ('addr', uintegerpattern())), can_have_label=True),                                              "j addr          -->  pc := addr",                  lambda m, params : '110'+str(uint2bin(m.group('addr'),12))+'0' ),
('j'    , pattern((('op', 'j'),    ('to', labelpattern())), can_have_label=True),                                                   "j label         -->  pc := addr",                  lambda m, params : '110'+str(label2bin(m.group('to'),12,params[1]))+'0' ),
('jal'  , pattern((('op', 'jal'),  ('addr', binarypattern(12))), can_have_label=True),                                              "jal addr        -->  r7 := pc+1 ; pc := addr",    lambda m, params : '110'+m.group('addr')+'1' ),
('jal'  , pattern((('op', 'jal'),  ('addr', uintegerpattern())), can_have_label=True),                                              "jal addr        -->  r7 := pc+1 ; pc := addr",    lambda m, params : '110'+str(uint2bin(m.group('addr'),12))+'1' ),
('jal'  , pattern((('op', 'jal'),  ('to', labelpattern())), can_have_label=True),                                                   "jal label       -->  r7 := pc+1 ; pc := addr",    lambda m, params : '110'+str(label2bin(m.group('to'),12,params[1]))+'1' ),
]
